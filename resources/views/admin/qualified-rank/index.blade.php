<x-layout.admin>
    <x-slot name="title">Qualified Ranks</x-slot>
                ...
                <table class="table table-hover display" id="product-table">
                    ...
                    <tbody>
                        @foreach($qualified as $rank)
                            <tr>
                                <td>{{ $rank["id"] }}</td>
                                <td>{{ $rank["date_added"] }}</td>
                                <td>{{ $rank["rank"]->name }}</td>
                                <td>{{ $rank["distributor_id"] }}</td>
                                <td>{{ $rank["distributor"] }}</td>
                                ...
                                <td>
                                    <span class="d-inline-block" tabindex="0" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="{{ __("view") }}">
                                        <a href="/{{App::currentLocale()}}/admin/qualified-ranks/{{ $rank["id"] }}" class="action-btn text-primary rounded">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                    </span>
                                    @if($rank["status"] === "PENDING")
                                        <span class="d-inline-block" tabindex="0" data-bs-toggle="popover" data-bs-trigger="hover focus" data-bs-content="{{ __("give_award") }}">
                                            <button class="action-btn text-success rounded" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                                onclick="setFormAction('/{{ App::currentLocale() }}/admin/qualified-ranks/{{ $rank['id'] }}/award')">
